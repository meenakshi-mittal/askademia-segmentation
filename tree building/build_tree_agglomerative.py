#!/usr/bin/env python3
"""
Build agglomerative summary trees from chunk files.

Identical flow to build_tree_gpt.py, except combining method:
  - Leaf GPT summaries.
  - Embed "<title>: <summary>" and adjacently merge most similar summaries
    until target_count = len/4 per level, max children per parent = 12.
  - No tiny-merge pass.
...
def _flatten_nodes(items: List[Any]) -> List[SummaryNode]:
    flat: List[SummaryNode] = []
    for it in items:
        if isinstance(it, list):
            flat.extend(_flatten_nodes(it))
        else:
            flat.append(it)
    return flat
...
def summarize_from_children(children: List[SummaryNode]) -> Tuple[str, str]:
    flat_children = _flatten_nodes(children)
    ...
    if not parts:
        for ch in flat_children:
            ...
            parts.append(f"- {title or 'Untitled'}: {summ}")
    text = "\n".join(parts)
    start_ts = flat_children[0].start_timestamp
    end_ts = flat_children[-1].end_timestamp
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build agglomerative trees from paper/data_avtext_fixed.")
